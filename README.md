...
### `InvalidPacket`
通常クライアントがブロックを破壊に関して送るパケットは、START_DIGGING→FINISHED_DIGGINGまたはSTART_DIGGING→CANCELLED_DIGGINGに限られます。それ以外のタイミングでFINISHED_DIGGINGパケットが送られてくることはないため、これらのパターンに合致した場合パケットの受け取りをキャンセルします。
## CivBreakとは？
### `ブロック破壊の仕組み`
